# -*- coding: utf-8 -*-
"""
PROGRAMA DE CÁLCULO DE PILARES — POLOTTO ENGENHARIA
Interface Streamlit (mobile-first). Motor de cálculo em motor_pilar.py.
...
<div class="pol-header">
  ...
  <h1>Cálculo de Pilares</h1>
  <div class="sub">Concreto armado · NBR 6118 · esbeltez + 2ª ordem
  (pilar-padrão) · flexo-compressão</div>
</div>
""", unsafe_allow_html=True)
...
l0 = c3.number_input("Altura livre l0 [m]", min_value=0.5, max_value=10.0,
                     ...
                     help="Adotado le = l0. Se a vinculação levar a le "
...
def fig_secao(res, opt):
    ...
    ax.text(bb / 2, hh + 1.6,
            f"{opt['texto']}  ·  estribo ø{opt['phi_t']:.1f} c/{opt['s_est']:.0f}",
