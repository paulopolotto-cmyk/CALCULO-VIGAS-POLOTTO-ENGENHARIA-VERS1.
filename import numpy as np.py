...
# ESTILIZAÇÃO AGRESSIVA REINSTALADA: Garante o layout maravilhoso anterior
st.markdown("""
    <style>
...
if len(st.session_state.lista_vaos) > 0:
    ...
    if st.session_state.res_calculo is not None:
        ...
        if "erro" in res: st.error(res["erro"])
            ...
            ax.text(posX_corte + 0.2, -0.65, f"{int(b_val)}", ha='center', va='top', fontsize=9, fontweight='bold')
            ax.text(posX_corte + 0.5, 0.0, f"{int(h_val)}", ha='left', va='center', fontsize=9, fontweight='bold')
            st.pyplot(fig)
